...
// Detection logic
$sports_cat_id  = ! empty( $codedokan['home_cat_9'] )         ? (int)$codedokan['home_cat_9']         : 0;
$lead_cat_id    = ! empty( $codedokan['home_lead_selected'] )  ? (int)$codedokan['home_lead_selected'] : 0;

// Fallback: if global $codedokan is empty, read directly from wp_options
if ( $sports_cat_id === 0 || $lead_cat_id === 0 ) {
    $possible_keys = array( 'codedokan', 'redux_demo', 'crimebangla', 'theme_options', 'crimebanglanews' );
    foreach ( $possible_keys as $_key ) {
        $_opt = get_option( $_key );
        if ( is_array( $_opt ) ) {
            if ( $sports_cat_id === 0 && ! empty( $_opt['home_cat_9'] ) ) {
                $sports_cat_id = (int) $_opt['home_cat_9'];
            }
            if ( $lead_cat_id === 0 && ! empty( $_opt['home_lead_selected'] ) ) {
                $lead_cat_id = (int) $_opt['home_lead_selected'];
            }
        }
        if ( $sports_cat_id > 0 && $lead_cat_id > 0 ) break;
    }
}

$sports_is_lead = ( $sports_cat_id !== 0 && $lead_cat_id !== 0 && $lead_cat_id === $sports_cat_id );

// Pre-fetch sports posts
$sports_posts = array();
$sports_total = 0;
if ( $sports_cat_id > 0 ) {
    $sports_pre_args = array(
        'cat'                    => $sports_cat_id,
        'posts_per_page'         => 10,
        'no_found_rows'          => true,
    ...
    $sports_pre_query = new WP_Query( $sports_pre_args );
    if ( $sports_pre_query->have_posts() ) {
        while ( $sports_pre_query->have_posts() ) {
            $sports_pre_query->the_post();
            $sports_posts[] = get_post();
        }
        wp_reset_postdata();
    }
    $sports_total = count( $sports_posts );
}

// Render function
if ( ! function_exists( 'render_premium_sports_portal' ) ) {
    function render_premium_sports_portal( $sports_cat_id, $sports_posts, $sports_total ) {
        global $post;
        if ( empty( $sports_posts ) ) {
            return;
        }
        $category_name = get_the_category_by_id( $sports_cat_id );
        $category_link = get_category_link( $sports_cat_id );
        
        // Split posts
        $hero_post = isset( $sports_posts[0] ) ? $sports_posts[0] : null;
        $left_posts = array_slice( $sports_posts, 1, 2 );
        $right_posts = array_slice( $sports_posts, 3, 3 );
        $bottom_posts = array_slice( $sports_posts, 6, 4 );
        ?>
        <style>
            .premium-sports-container {
            ...
            .sports-card:hover .sports-card-img-wrapper img {
            ...
            /* Hero Card Specifics */
            .sports-hero-card {
                background: linear-gradient(to bottom, transparent 40%, rgba(3, 7, 18, 0.95) 100%);
            }
            .sports-hero-title {
                font-size: 22px;
                line-height: 1.3;
        ...
        <div class="premium-sports-container" id="sports-news">
            <div class="sports-header">
                <div class="sports-title-wrapper">
                    <i class="fas fa-trophy sports-icon"></i>
                    <h3 class="sports-title"><?php echo esc_html( $category_name ); ?></h3>
                </div>
                <a href="<?php echo esc_url( $category_link ); ?>" class="sports-more-btn">
                    সব খবর <i class="fas fa-arrow-right"></i>
                </a>
            </div>
            
            <!-- Main Grid: Left, Center (Hero), Right -->
            <div class="row">
                <!-- Left Column (2 Medium Cards) -->
                <div class="col-lg-3 col-md-12 mb-4 mb-lg-0">
                    <div class="row h-100">
                        <?php foreach ( $left_posts as $p ) : ?>
                            <div class="col-12 col-md-6 col-lg-12 mb-3">
                                <div class="sports-card">
                                    <div class="sports-card-img-wrapper">
                ...
                <!-- Center Column (Big Hero Card) -->
                <div class="col-lg-6 col-md-12 mb-4 mb-lg-0">
                    <?php if ( $hero_post ) : ?>
                        <div class="sports-card h-100">
                            <div class="sports-card-img-wrapper" style="padding-top: 60%; height: 100%;">
                                <span class="sports-badge" style="background:#dc2626;">HOT</span>
                                <?php if ( has_post_thumbnail( $hero_post->ID ) ) : ?>
                                    <?php echo get_the_post_thumbnail( $hero_post->ID, 'large', array('style' => 'height:100%; object-fit:cover;') ); ?>
                                <?php else : ?>
                                    <img src="<?php echo get_template_directory_uri(); ?>/assets/images/default.jpg" alt="">
                                <?php endif; ?>
                            </div>
                            <div class="sports-card-content" style="position: absolute; bottom: 0; left: 0; right: 0; background: linear-gradient(to top, rgba(0,0,0,0.9) 0%, rgba(0,0,0,0.4) 70%, transparent 100%); padding: 30px 20px 20px 20px;">
                                <h4 class="sports-card-title sports-hero-title">
                                    <a href="<?php echo get_permalink( $hero_post->ID ); ?>" style="color:#ffffff; text-shadow: 1px 1px 3px rgba(0,0,0,0.8);"><?php echo esc_html( $hero_post->post_title ); ?></a>
                                </h4>
                                <p class="sports-card-excerpt" style="color:#cbd5e1; text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">
                                    <?php
                                    $excerpt = get_the_excerpt($hero_post->ID);
                    ...
                    <?php foreach ( $bottom_posts as $p ) : ?>
                        <div class="col-12 col-sm-6 col-lg-3 mb-3 mb-lg-0">
                            <div class="sports-card">
                                ...
                                <div class="sports-card-content">
                                    <h4 class="sports-card-title" style="font-size:14px;">
                                        <a href="<?php echo get_permalink( $p->ID ); ?>"><?php echo esc_html( $p->post_title ); ?></a>
                                    </h4>
                                </div>
                            </div>
...
<?php if ( $sports_is_lead && $sports_total > 0 ) : ?>
    <!-- 1. Premium Sports Section (Full Width Hero) -->
    <section id="top-lead-content-sports" class="bg-white pt-3">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <?php render_premium_sports_portal( $sports_cat_id, $sports_posts, $sports_total ); ?>
                </div>
            </div>
        </div>
    </section>

    <!-- 2. Selected News (International) and Sidebar side-by-side below the Sports Section -->
    <section id="top-lead-content-sub-sidebar" class="bg-white pb-3">
        <div class="container-fluid">
            <div class="row">
